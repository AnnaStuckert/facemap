...
class ViT_Seg(nn.Module):
    def __init__(self, pretrained=True):
        ...
        self.conv_out = nn.Conv2d(in_channels=768, out_channels=1, kernel_size=1)

        # self.conv_out = nn.Conv2d(
        # in_channels=1024, out_channels=1, kernel_size=1
        # )  # Corrected input channels to 1024, kernel size 7 because that is what the model is trained with!

    def forward(self, x):
        # Extract features from the last stage of Swin
        x = self.encoder(x)[
            0
        ]  # Access the last feature map with appropriate dimensions
